// Script to convert existing necDatabase to new schema
// Run with: node scripts/generate-nec-data.js

const fs = require('fs');
const path = require('path');

// Copy of necDatabase from reference-tab.tsx
const necDatabase = [
  {
    article: "110.26",
...
const newArticles = necDatabase.map(item => {
  // Convert keyPoints to content paragraphs
  ...
  // Add common violations as separate content sections
...
const outputPath = path.join(__dirname, '../lib/nec-data.json');
fs.writeFileSync(outputPath, JSON.stringify(output, null, 2));
console.log(`Generated ${newArticles.length} articles at ${outputPath}`);

// Also generate a TypeScript file exporting the data
const tsContent = `// Generated NEC 2023 dataset
import { NECArticle } from './nec-data';

export const NEC_ARTICLES: NECArticle[] = ${JSON.stringify(newArticles, null, 2)};

export default NEC_ARTICLES;
`;
const tsPath = path.join(__dirname, '../lib/nec-data-generated.ts');
fs.writeFileSync(tsPath, tsContent);
console.log(`Generated TypeScript file at ${tsPath}`);